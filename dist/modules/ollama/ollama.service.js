...
let OllamaService = OllamaService_1 = class OllamaService {
    ...
    async analyzeReflection(title, content, description, nearbyReportsContext) {
...
- Nếu hợp lệ, ghi lý do ngắn gọn (ví dụ: "Nội dung phản ánh hợp lệ").`,
                ...
                this.logger.log(`Gemini phân tích xong trong ${elapsed}s. Raw: ${JSON.stringify(aiResponse)}`);
                return this.parseResponse(aiResponse);
            }
            catch (err) {
                const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
                this.logger.error(`Gemini lỗi/timeout sau ${elapsed}s: ${err.message}.`);
                return this.fallbackResult();
            }
        }
        try {
            const response = await fetch(`${this.baseUrl}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a Vietnamese flood-report content moderator. Analyze the report and reply with ONLY valid JSON, no markdown, no explanation.',
                        },
                        {
                            role: 'user',
                            content: userContent,
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    this.logger.warn(`Model "${this.model}" chưa được pull. Chạy lệnh: ollama pull ${this.model}`);
                }
                else {
                    this.logger.warn(`Ollama API lỗi HTTP ${response.status}, bỏ qua kiểm duyệt`);
                }
                return this.fallbackResult();
            }
            const data = (await response.json());
            ...
            this.logger.log(`Ollama phân tích xong trong ${elapsed}s. Raw: ${JSON.stringify(aiResponse)}`);
            return this.parseResponse(aiResponse);
        }
        catch (err) {
            const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
            this.logger.warn(`Ollama timeout/lỗi sau ${elapsed}s: ${err.message}. Bỏ qua kiểm duyệt.`);
            return this.fallbackResult();
        }
    }
    parseResponse(raw) {
